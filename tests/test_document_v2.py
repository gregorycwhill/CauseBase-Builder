...
def test_document_v2_is_cached_and_has_page_provenance(tmp_path):
    ...
    assert first["pages"][0]["route"] in {"ocr","ocr_unavailable","native_text_layout"}
...
def test_benchmark_emits_machine_and_human_reports(tmp_path):
    ...
    assert report["results"][0]["reason"]=="no_private_document_required"
    assert (tmp_path/"runtime"/"golden-corpus-v1-results.json").exists()
    assert (tmp_path/"runtime"/"golden-corpus-v1-results.md").exists()
